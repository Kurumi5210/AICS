...
class FullyConnectedLayer(object):
    ...
    def forward(self, input): # 前向传播计算
        ...
        self.output= self.input @ self.weight + self.bias   # Y = XW + b
        return self.output

    def backward(self, top_diff):   # 反向传播的计算
        # TODO：全连接层的反向传播，计算参数梯度和本层损失
        self.d_weight= (self.input).T @ top_diff    
        # self.d_bias= np.sum(top_diff, axis=0, keepdim=True)
...
class ReLULayer(object):
    ...
    def forward(self, input):  # 前向传播的计算
        ...
        # output= max(0, self.input)
        output = np.maximum(0, self.input)  # 使用 np.maximum 实现 ReLU
        return output
    def backward(self, top_diff):   # 反向传播的计算
...
class SoftmaxLossLayer(object):
    ...
    def forward(self, input):  # 前向传播的计算
        ...
        input_exp = np.exp(input-input_max) # 减去最大值后取指数
        exp_sum = np.sum(input_exp, axis=1, keepdims=True)  # 每行的指数和
        self.prob = input_exp / exp_sum # softmax 概率
        return self.prob
